...
bool OK = 0;
int sinPWM[] =
{ 1, 2, 5, 7, 10, 12, 15, 17, 19, 22, 24, 27, 30,
  32, 34, 37, 39, 42,
  44, 47, 49, 52, 54, 57, 59, 61, 64, 66, 69,
  71, 73, 76, 78, 80, 83, 85, 88, 90, 92, 94,
  97, 99,
  101, 103, 106, 108, 110, 113, 115, 117,
  119, 121, 124, 126, 128, 130, 132, 134,
  136, 138, 140, 142, 144, 146,
  148, 150, 152, 154, 156, 158, 160, 162,
  164, 166, 168, 169, 171, 173, 175, 177,
  178, 180, 182, 184, 185, 187, 188, 190,
  192, 193,
  ...
  236, 237, 237, 238, 239, 240, 240, 241,
  ...
  241, 240, 240, 239, 238, 237, 237, 236,
  235, 234,
  233, 232, 231, 230, 229, 228, 227, 226,
  225, 224, 223, 222, 221, 220, 219, 217,
  216, 215, 213, 212, 211, 209, 208, 207,
  205, 204,
  202, 201, 199, 198, 196, 195, 193, 192,
  190, 188, 187, 185, 184, 182, 180, 178,
  177, 175, 173, 171, 169, 168, 166, 164,
  162, 160,
  158, 156, 154, 152, 150, 148, 146, 144,
  142, 140, 138, 136, 134, 132, 130, 128,
  126, 124, 121, 119, 117, 115, 113, 110,
  108, 106,
  103, 101, 99, 97, 94, 92, 90, 88, 85, 83,
  80, 78, 76, 73, 71, 69, 66, 64, 61, 59, 57,
  54, 52, 49, 47, 44, 42, 39, 37, 34, 32, 30,
...
ISR(TIMER1_COMPA_vect) { //interrupt when timer 1 match with OCR1A value
  ...
  if (!OK) {
    ...
    OCR0A = sinPWM[i]; //take the value from vector corresponding to position i(i is zero indexed)//enable pin 6 to corresponding duty cycle
    OCR2B = 0; //makekl pin 9 0
    OCR2A = sinPWM[i]; //enable pin 10 to corresponding duty cycle
  }
  else if (OK) {
    OCR0A = 0; //make pin 6 0
    OCR0B = sinPWM[i]; //take the value from vector corresponding to position i(i is zero indexed)//enable pin 5 to corresponding duty cycle
    OCR2A = 0; //make pin 10 0
    OCR2B = sinPWM[i]; //enable pin 9 to corresponding duty cycle
  }
