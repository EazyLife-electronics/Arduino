...
void setup() {
  ...
  Serial.begin(9600);
  //EEPROM.write(1, 0);
  DDRD  |=  B00011100;           //0x38    Configure pins 2, 3 and 4 as outputs
  PORTD  = 0;           //0x00 pins 0 to 7 set to LOW
  DDRB  |= B00001110;           //0x0E    Configure pins 9, 10 and 11 as outputs
  PORTB  &= B00110001;          //0x31    D9, D10 and D11 to LOW
  // Timer1 module setting: set clock source to clkI/O / 1 (no prescaling)
  TCCR1A = 0;
  TCCR1B = B00000001;      //0x01
  // Timer2 module setting: set clock source to clkI/O / 1 (no prescaling)
  TCCR2A = 0;
  TCCR2B = B00000001;      //0x01
  // Analog comparator setting
  ACSR   = B00010000;           // 0x10    Disable and clear (flag bit) analog comparator interrupt
...
void loop() {
  ...
  if (MOTOR_SPINNING)
  {
    ...
    ACSR |= B00001000;                    //0x08    Enable analog comparator interrupt
  ...
  if (!MOTOR_SPINNING)
  {
    ...
    if (full_stop)
    {
      if (currentMillis - previousMillis >= 2000)
      {
        ...
        TCCR2A =  0;
        TCCR1A =  0;
        PORTD  = 0;            // 0x00    pins 0 to 7 set to LOW //stop everything
        PORTB  &= B00110001;           //0x31    D9, D10 and D11 to LOW
        OCR1A = beeping_PWM_VALUE;
    ...
    else
    {
      ...
      TCCR2A =  0;
      TCCR1A =  0;
      PORTD  = 0;            //B00000000    pins 0 to 7 set to LOW //stop everything
      PORTB  = 0;           //B00000000    D9, D10 and D11 to LOW
    }
  }//end of if !MOTOR_SPINNING
...
void BEMF_A_RISING() {
  ...
  ACSR |= B00000011;        //0x03    Set interrupt on rising edge*/
}
void BEMF_A_FALLING() {
  ADCSRA = (0 << ADEN);     // Disable the ADC module
  ADCSRB = (1 << ACME);     // Enable MUX select for negative input of comparator
  ADMUX = 2;                // Select A2 as comparator negative input
  ACSR &= ~B00000001;       //0x01    Set interrupt on falling edge*/
}
void BEMF_B_RISING() {
  ADCSRA = (0 << ADEN);     // Disable the ADC module
  ADCSRB = (1 << ACME);     // Enable MUX select for negative input of comparator
...
void AH_BL() {
  ...
  TCCR2A =  0;            //OC2A - D11 normal port.
  TCCR1A =  10000001;         //0x81    OC1A - D9 compare match noninverting mode, downcounting ,PWM 8-bit
}
//D9 PWM and D3 HIGH
void AH_CL() {
...
void BH_CL() {
  PORTD = B00001000;      //Set D3 to HIGH and the rest to LOW
  TCCR2A =  0;            //OC2A - D11 normal port.
  TCCR1A =  B00100001;         //0x21    OC1B - D10 compare match noninverting mode, downcounting ,PWM 8-bit
}
//D10 PWM and D5 HIGH
void BH_AL() {
...
void CH_AL() {
  PORTD = B00100000;      //Set D5 to HIGH and the rest to LOW
  TCCR1A =  0;            // OC1A and OC1B normal port
  TCCR2A =  10000001;         //0x81    OC2A - D11 compare match noninverting mode, downcounting ,PWM 8-bit
}
//D11 PWM and D4 HIGH
void CH_BL() {
